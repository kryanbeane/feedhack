...
{#if hasPassword}
	<SettingsModule
		...
		formTarget="/home/api?/updatePassword"
		fields={[
			{
				id: "newPassword1",
				label: "New Password",
	...
	<div class="card mt-8 flex max-w-md flex-col p-6 pb-7 shadow md:flex-row">
		<div class="flex flex-col gap-y-4">
			{#if usingOAuth}
				...
				<div>
					You use oAuth to sign in ("Sign in with Github" or similar).
					You can continue to access your account using only oAuth if
					you like!
				</div>
			{:else}
			...
			{/if}
			<div>
				The button below will send you an email at {session?.user
					?.email} which will allow you to set your password.
			</div>
			<button
			...
			<div
				class="success alert alert-success {sentEmail ? '' : 'hidden'}"
			>
